...
export class AuthService {
  ...
  private async issue(user: User, ipAddress?: string, userAgent?: string): Promise<SessionResult> {
    ...
    return { accessToken, refreshToken, user: this.safe(user) };
  }
  async login(dto: LoginDto, ip?: string, agent?: string) {
    ...
    return this.issue(user, ip, agent);
  }
  async refresh(token: string | undefined, ip?: string, agent?: string) {
    if (!token) throw new UnauthorizedException('Refresh token required');
    const session = await this.prisma.refreshSession.findUnique({
      where: { tokenHash: this.hash(token) },
      include: { user: { include: { couple: { select: { status: true, deletedAt: true } } } } },
    });
    ...
    this.assertAccess(session.user);
    const next = await this.issue(session.user, ip, agent);
    const replacement = await this.prisma.refreshSession.findUniqueOrThrow({
